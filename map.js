...
function resolveClientCoords(client) {
  ...
  if (manualCoords[key]) {
    const coords = normalizeCoordPair(manualCoords[key].lat, manualCoords[key].lng);
    if (validCoords(coords)) return { coords, source: "manual", label: manualCoords[key].label };
  }

  return databaseLocation(client);
}

function databaseLocation(client) {
  // Ordre strict pour la carte : manual_lat/manual_lng, puis web_lat/web_lng.
  // Important : les valeurs nulles/vides ne doivent jamais être converties en 0,0.
  ...
  // Compatible avec une vue/RPC Supabase qui renvoie :
  // COALESCE(manual_lat, web_lat) AS lat,
  // COALESCE(manual_lng, web_lng) AS lng
  const coalesced = coordsFrom(client, ["lat"], ["lng"]);
  if (coalesced) return { coords: coalesced, source: "database", label: "coordonnées Supabase" };
...
function coordsFrom(client, latFields, lngFields) {
  for (const latField of latFields) {
    for (const lngField of lngFields) {
      if (!hasCoordValue(client[latField]) || !hasCoordValue(client[lngField])) continue;
      const coords = normalizeCoordPair(client[latField], client[lngField]);
...
function setUserMarker(coords) {
  ...
  if (!userMarker) {
    userMarker = L.marker(coords, {
      interactive: false,
      icon: L.divIcon({ className: "user-arrow", html: "<span></span>", iconSize: [42, 42], iconAnchor: [21, 21] })
    }).addTo(map);
    return;
  }
...
function normalizeCoordPair(lat, lng) {
  const coords = [toCoordNumber(lat), toCoordNumber(lng)];
  if (!validCoords(coords)) return coords;
...
function sourceBadge(source) { return ({ manual: "G", web: "W", database: "B" })[source] || "?"; }
function heatClass(client) {
  if (client.signed_client || client.machine_status === "signe") return "heat-signed";
  return needsAction(client) ? "heat-action" : "heat-froid";
}
function confidenceClass() { return ""; }
function needsAction(client) {
  if (["tres-chaud", "chaud", "tiede"].includes(client.heat_level)) return true;
  if (isDueDate(client.next_action_at)) return true;
...
function heatWeight(client) {
  if (client.signed_client || client.machine_status === "signe") return 5;
  return needsAction(client) ? 3 : 1;
}
function shouldAlert(client) { return needsAction(client); }
