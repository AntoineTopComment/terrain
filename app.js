...
function toggleLiveMode() {
  ...
  if (state.liveTracking) {
    ...
    if (coords) {
      positionMiniMap(coords, getLocatedClients(120), { force: true });
    } else {
      refreshUserPosition(handlePositionChange)
        .then((freshCoords) => positionMiniMap(freshCoords, getLocatedClients(120), { force: true }))
...
function homeMarkerClass(client) {
  if (state.quests.some((quest) => quest.target_client_id && String(quest.target_client_id) === String(client.id) && !quest.completed)) return "poi-quest";
...
function homeMarkerIcon(client) {
  if (state.quests.some((quest) => quest.target_client_id && String(quest.target_client_id) === String(client.id) && !quest.completed)) return "!";
  if (isDue(client.next_action_at)) return "!";
...
function placeSelectedOnMap() {
  ...
  enableManualPlacement(state.selectedClient, () => {
